...
function evalSnippet(src, snippetDiv) {
  ...
  fetch(`${PLAYGROUND_HOST}/run`, {
    ...
    if (!data.success) {
      snippetDiv.innerHTML = `Error: ${(0, import_escape_html.default)(data.error || "Unknown error")}`;
      return;
    }
    ...
    for (const item of data.results) {
      if ("printed" in item) {
        outputParts.push(
          `<span class="stdout">${(0, import_escape_html.default)(item.printed.s)}</span>`
        );
      } else if ("printed_stderr" in item) {
        outputParts.push(
          `<span class="stderr">${(0, import_escape_html.default)(item.printed_stderr.s)}</span>`
        );
      } else if ("error" in item || "value" in item) {
        const result = item;
        if (result.error) {
          snippetDiv.innerHTML = `Error: ${(0, import_escape_html.default)(result.error)}`;
          hasError = true;
          break;
        }
        if (result.value) {
          valueParts.push(
            `<span class="playground-value">${(0, import_escape_html.default)(result.value)}</span>`
          );
        }
      }
    }
    ...
    snippetDiv.innerHTML = `Fetch error: ${(0, import_escape_html.default)(message)}`;
  });
}
function formatSnippet(src, onFormatted, onError) {
  fetch(`${PLAYGROUND_HOST}/format`, {
    ...
    if (!data.success || data.formatted === void 0) {
...
function fetchDiagnostics(src, onDiagnostics) {
...
var import_escape_html, PLAYGROUND_HOST;
var init_api = __esm({
  "api.ts"() {
    "use strict";
    import_escape_html = __toESM(require_escape_html());
    PLAYGROUND_HOST = "https://playground.garden-lang.org";
  }
});
...
var SelectedDiagnostic, LintState, setDiagnosticsEffect, togglePanel, movePanelSelection, lintState, activeMark, closeLintPanel, lintPlugin, lintConfig, DiagnosticWidget, PanelItem, LintPanel, baseTheme2, lintExtensions;
...
// language.ts
var gardenLanguage, gardenHighlighting;
var init_language = __esm({
  "language.ts"() {
    ...
    init_dist4();
    init_dist5();
    gardenLanguage = StreamLanguage.define({
      token(stream) {
        if (stream.match("//")) {
          stream.skipToEnd();
          return "comment";
    ...
    gardenHighlighting = syntaxHighlighting(
...
// editor.ts
function createGardenEditor(doc2, onDocChanged) {
...
function setEditorDiagnostics(view, diagnostics) {
  const doc2 = view.state.doc;
  const lineCount = doc2.lines;
...
// snippets.ts
function resetSnippet(snippetDiv, editorView, originalCodeNode, editButton) {
    ...
    if (editButton) {
      ...
      editButton.addEventListener("click", (_e) => {
        ...
        if (editorView) {
          ...
          originalCodeNode = codeNode.cloneNode(true);
...
var init_snippets = __esm({
  "snippets.ts"() {
    ...
    init_api();
    init_editor();
    editorViews = /* @__PURE__ */ new WeakMap();
  }
});
...
var require_playground = __commonJS({
  "playground.ts"() {
    ...
    function setupPlayground() {
      ...
      if (playgroundRunButton && playgroundFormatButton && playgroundOutput && playgroundOutput instanceof HTMLElement && playgroundEditor && playgroundEditor instanceof HTMLTextAreaElement) {
        const initialContent = playgroundEditor.value + "\n\n";
        let checkTimer;
        const scheduleCheck = (src) => {
...
export default require_playground();

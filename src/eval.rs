...
pub(crate) fn eval_up_to_param(
    ...
    for item in items {
        ...
        match &def.2 {
            Definition_::Fun(name_sym, fun_info, _, _) => {
...
pub(crate) fn eval_toplevel_call(
    ...
    env.push_expr_to_eval(ExpressionState::EvaluatedSubexpressions, call_expr);
...
fn eval_defs(definitions: &[Definition], env: &mut Env) -> ToplevelEvalSummary {
    ...
    for definition in definitions {
        match &definition.2 {
            Definition_::Fun(name_symbol, fun_info, _, _) => {
...
fn eval_expr(
    ...
    match &outer_expr.expr_ {
        ...
        Expression_::MethodCall(receiver_expr, meth_name, paren_args) => {
            if expr_state.done_children() {
                match eval_method_call(env, expr_value_is_used, outer_expr, meth_name, paren_args)?
                {
                    Some(new_stack_frame) => {
                        return Ok(Some(new_stack_frame));
                    }
...
pub(crate) fn eval(env: &mut Env, session: &Session) -> Result<Value, EvalError> {
    ...
    loop {
        if let Some((mut expr_state, outer_expr)) = env.current_frame_mut().exprs_to_eval.pop() {
            ...
            match eval_expr(env, session, &outer_expr, &mut expr_state) {
...
fn eval_block(env: &mut Env, expr_value_is_used: bool, block: &Block) {
    ...
    for expr in block.exprs.iter().rev() {
        ...
        env.push_expr_to_eval(ExpressionState::NotEvaluated, expr);
    }
}
...
mod tests {
    ...
    fn parse_defs_from_str(src: &str, id_gen: &mut SyntaxIdGenerator) -> Vec<Definition> {
    ...
    fn parse_exprs_from_str(src: &str, id_gen: &mut SyntaxIdGenerator) -> Vec<Expression> {
        let (items, errors) = parse_toplevel_items(&PathBuf::from("__test.gdn"), src, id_gen);
        assert!(errors.is_empty());
    ...
    fn test_eval_persist_env() {
        ...
        let exprs = parse_exprs_from_str("let foo = 123", &mut id_gen);
        eval_exprs(&exprs, &mut env).unwrap();

        let exprs = parse_exprs_from_str("foo", &mut id_gen);
        eval_exprs(&exprs, &mut env).unwrap();
    }

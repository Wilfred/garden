    ...
    for definition in definitions {
        match &definition.2 {
            ...
            Definition_::Enum(enum_info) => {
                ...
                env.types
                    .insert(enum_info.name.sym.name.clone(), Type::Enum(enum_info.clone()));
                ...
                for (idx, variant_sym) in enum_info.variants.iter().enumerate() {
                    ...
                    env.set_with_file_scope(
                        ...
                        Value::Enum(enum_info.name.sym.name.clone(), idx, None),
                    );
                }

                let name_as_sym = SymbolName(enum_info.name.sym.name.name.clone());
